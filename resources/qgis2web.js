...
function captureSelectedPlanMap(feature, callback) {
    var exportFeature = feature.clone();
    exportFeature.setGeometry(feature.getGeometry().clone());
    selectedFeatureOverlayCollection.clear();
    selectedFeatureOverlayCollection.push(exportFeature);
    var view = map.getView();
    var previousRotation = view.getRotation ? view.getRotation() : 0;
    if (view.setRotation) {
        view.setRotation(0);
    }
    ...
    layersList.forEach(function(layer) {
        layersToRestore.push({ layer: layer, visible: layer.getVisible() });
        layer.setVisible(layer === lyr_GoogleSatellite_0 || layer === lyr_DelimitacionesDGZFMT_5);
    });
    var overlaysToRestore = [
        { layer: selectedFeatureOverlayLayer, visible: selectedFeatureOverlayLayer.getVisible() },
        { layer: searchSelectionOverlay, visible: searchSelectionOverlay.getVisible() }
    ];
    selectedFeatureOverlayLayer.setVisible(true);
    searchSelectionOverlay.setVisible(false);
    ...
    getMapCanvasDataURL(function(imgData) {
        if (view.setRotation) {
            view.setRotation(previousRotation);
        }
        layersToRestore.forEach(function(item) {
            item.layer.setVisible(item.visible);
        });
        overlaysToRestore.forEach(function(item) {
            item.layer.setVisible(item.visible);
        });
        callback(imgData);
    });
...
function exportSelectedPlanPDF(title) {
    ...
    captureSelectedPlanMap(selectedPlanFeature, function(imgData) {
            if (!imgData) {
                imgData = createFeatureCanvasDataURL(selectedPlanFeature, 1400, 820);
            }
            if (!imgData) {
            ...
            doc.text('Polígono seleccionado 2D', margin + 470, 66);
            ...
            updateSearchMessage('export-plan-message', 'Plano del polígono seleccionado generado.');
    });
}
...
function populateDatalist(listId, values, filter) {
    ...
    for (var i = 0; i < values.length && i < 50; i++) {
        var text = values[i];
        if (!filter || normalizeSearchValue(text).indexOf(filter) !== -1) {
...
function findFeatureByField(layer, fieldName, searchValue) {
    ...
    var features = source.getFeatures();
    var aliases = [];
    var baseName = fieldName.toString();
    aliases.push(baseName);
    aliases.push(baseName.replace(/\s+/g, ' ').trim());
    if (/conces/i.test(baseName)) {
        aliases.push('No. Conces');
        aliases.push('No. Concesión');
        aliases.push('No. Concesion');
        aliases.push('No Conces');
        aliases.push('Concesion_');
    }
    if (/clave/i.test(baseName)) {
        aliases.push('Clave');
    }
    for (var i = 0; i < features.length; i++) {
        var feature = features[i];
        ...
        for (var j = 0; j < aliases.length; j++) {
            var alias = aliases[j];
            var fieldValue = feature.get(alias);
            if (fieldValue === undefined || fieldValue === null) {
                continue;
            }
            var fieldText = normalizeSearchValue(fieldValue);
            if (fieldText === term || fieldText.indexOf(term) !== -1 || term.indexOf(fieldText) !== -1) {
                return feature;
            }
        }
        for (var k = 0; k < keys.length; k++) {
            ...
            var keyLower = normalizeSearchValue(key);
            var mustCheckKey = keyLower.indexOf(normalizeSearchValue(baseName)) !== -1 || keyLower.indexOf(term) !== -1;
            if (!mustCheckKey) {
                continue;
            }
            var alternateValue = feature.get(key);
            if (alternateValue !== undefined && alternateValue !== null) {
                var alternateText = normalizeSearchValue(alternateValue);
                if (alternateText === term || alternateText.indexOf(term) !== -1 || term.indexOf(alternateText) !== -1) {
                    return feature;
                }
            }
        }
...
function showFeaturePopup(feature, layer, coord) {
    var activeLayer = layer || selectedPlanLayer;
    if (!feature || !activeLayer) {
        return;
    }
    var keys = feature.getKeys ? feature.getKeys() : [];
    var title = activeLayer.get('popuplayertitle') || 'Seleccionado';
    var popupHtml = '<ul><li><table><a><b>' + title + '</b></a>';
    popupHtml += createPopupField(feature, keys, activeLayer);
    popupHtml += '</table></li></ul>';
    popupContent = popupHtml;
    popupCoord = coord || getFeaturePopupCoordinate(feature);
    updatePopup();
}

function getFeaturePopupCoordinate(feature) {
    ...
    if (geometryType === 'Polygon' && geometry.getInteriorPoint) {
...
function zoomToSearchFeature(feature, layer) {
    ...
    if (selectedPlanFeature && selectedPlanFeature.getGeometry()) {
        var popupCoord = getFeaturePopupCoordinate(selectedPlanFeature);
        showFeaturePopup(selectedPlanFeature, selectedPlanLayer, popupCoord);
    }
    var view = map.getView();
